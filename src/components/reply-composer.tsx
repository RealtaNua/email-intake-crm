...
import { DEMO_THREAD_MESSAGE } from "@/lib/types";
 ...
 */
function SendButton({ to, canSend }: { to: string; canSend: boolean }) {
    ...
    <div className="flex flex-wrap items-center gap-3">
      ...
      <span className="text-xs text-ink-muted">
        {canSend ? (
          ...
          "Demo thread — nothing will be sent."
        )}
      </span>
    </div>
  );
...
export function ReplyComposer({
  ...
  /** False for a thread with no genuinely delivered message in it. */
  canSend: boolean;
  ...
  const blocked = useRef<HTMLDialogElement>(null);
  ...
  return (
    <div className="mt-4 rounded-xl bg-page p-4">
      ...
      {/* Stopped in the browser so the refusal is immediate and obvious. The
          server refuses independently — this dialog is the explanation, not
          the enforcement. */}
      <dialog
        ref={blocked}
        className="max-w-md rounded-2xl p-6 text-ink shadow-xl backdrop:bg-ink/40"
      >
        <h4 className="text-sm font-semibold">Can&apos;t reply to this thread</h4>
        <p className="mt-2 text-sm text-ink-muted">{DEMO_THREAD_MESSAGE}</p>
        <button
          type="button"
          onClick={() => blocked.current?.close()}
          className="mt-4 rounded-lg bg-brand px-3.5 py-2 text-sm font-medium text-white hover:bg-brand-deep"
        >
          Got it
        </button>
      </dialog>
      ...
      >
        ...
        <textarea
          ...
          className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-slate-900"
        />
        <SendButton to={to} canSend={canSend} />
      </form>
    </div>
  );
}
